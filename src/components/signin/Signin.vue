...
<form class="signinForm" ref="signinForm">
  ...
  <a v-show="signType === 'signin'" class="signup" href="#" @click.prevent="switchSignup">没有账号？点击住蹙额</a>
  <a v-show="signType === 'signup'" class="signup" href="#" @click.prevent="switchSignup">已有账号返回登录</a>
</form>
</template>
<script>
  ...
  methods :{
    ...
    sign(type){
      ...
      this.$store.dispatch('SIGN_BY_USERNAME',formInfo).then((data) => {
        ...
        }else{//为false的时候，既验证失败，将按钮重新置为可用
          this.isLoading = false;
          submitButton.disabled = '';
        }
      }).catch( err => {
